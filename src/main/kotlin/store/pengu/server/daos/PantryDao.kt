...
class PantryDao(
    ...
    fun getPantries(create: DSLContext = dslContext): List<Pantry_details> {
                ...
                Pantry_details(
                    ...
                    product_num = create.fetchCount(
                        DSL.select()
                            ...
                            .where(PANTRIES.PANTRY_ID.eq(it[PANTRIES.PANTRY_ID]))
                    )
                )
            }
    ...
    fun addProductToPantry(product_x_pantry: Product_x_Pantry, create: DSLContext = dslContext): Boolean {
        return create.insertInto(
            PRODUCT_X_PANTRY,
            PRODUCT_X_PANTRY.PRODUCT_ID,
            PRODUCT_X_PANTRY.PANTRY_ID,
            PRODUCT_X_PANTRY.WANT_QTY,
            PRODUCT_X_PANTRY.HAVE_QTY
        )
            .values(
                product_x_pantry.product_id,
                product_x_pantry.pantry_id,
